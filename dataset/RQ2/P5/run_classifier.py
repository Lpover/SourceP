...
# initialize logger
logging.config.fileConfig("logging_info.cfg")
    ...
    def __init__(self, tokenizer, args, file_path='train'):
        ...
        with open('/'.join(index_filename.split('/')[:-1])+'/data.jsonl') as f:
            for line in f:
                line = line.strip()
                js = json.loads(line)
                url_to_code[js['idx']] = js['sourcecode']
        ...
        # use 100% valid data to keep best model
        if 'valid' in file_path:
            data = random.sample(data, int(len(data)*1))
...
def main():
    ...
    model.to(args.device)
    test(args, model, tokenizer, best_threshold=0.003)
